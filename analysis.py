...
def _badges(r):
    out = []
    if r.get("site"):
        out.append('<span class="bd bd-b">사이트</span>')
    if r.get("blog"):
        out.append('<span class="bd bd-b">네이버블로그</span>')
    if r.get("youtube"):
        label = "유튜브(우리채널)" if r.get("youtube_ours") else "유튜브"
        out.append(f'<span class="bd bd-r">{label}</span>')
    return "".join(out)
...
def _rank_txt(r):
    if r.get("err"):
        return ""
    if r.get("rank"):
        return f'<span class="rk">우리 {r["rank"]}번째 · 총 {r["total"]}곳</span>'
    if r.get("exposed"):
        return '<span class="rk">순위 산정 불가</span>'
    return ""


def _platform_block(r, aliases):
    exposed = r.get("exposed")
    pill = ('<span class="on">노출</span>' if exposed else '<span class="off">미노출</span>')
    badges = _badges(r)
    rank = _rank_txt(r)
    others = r.get("others") or []
    others_html = (f'<div class="oth">함께 언급: {_html.escape(", ".join(others))}</div>'
                   if others else "")
    if r.get("err"):
        ans = f'<div class="ans err">{_html.escape(r["err"])}</div>'
    else:
        ans = f'<div class="ans">{_hl(r.get("excerpt", ""), aliases)}</div>'
    return (f'<div class="prow"><span class="pl">{r["plat"]}</span>'
            f'<div class="pbody">{pill}{badges}{rank}{ans}{others_html}</div></div>')


def card_html(query, rows, aliases):
    inner = "".join(_platform_block(r, aliases) for r in rows)
    return (f'<div class="qcard"><div class="qt">{_html.escape(query)}</div>{inner}</div>')
...
def summary_html(rows_by_query, aliases):
    ...
    for label, c in sov[:8]:
        me = " me" if label == "우리 병원" else ""
        w = int(c / maxc * 100)
        bars += (f'<div class="bar{me}"><span class="bn">{_html.escape(label)}</span>'
    ...
    chips = "".join(f'<span class="chip">{_html.escape(d)}<b>{c}</b></span>'
                    for d, c in doms)
    if not chips:
        chips = '<div style="color:#999;font-size:13px">데이터 없음</div>'

    return (f'<div class="sum"><div class="sgrid">'
            f'<div class="scard"><div class="lb">GPT 발견형 노출</div>'
            f'<div class="nm">{g[0]}<small> / {g[1]} · {g[2]}%</small></div></div>'
            f'<div class="scard"><div class="lb">Gemini 발견형 노출</div>'
            f'<div class="nm">{m[0]}<small> / {m[1]} · {m[2]}%</small></div></div></div>'
            f'<div class="sec">노출 점유율 (등장 횟수)</div>{bars}'
            f'<div class="sec">AI가 자주 인용한 출처</div><div class="chips">{chips}</div></div>')
